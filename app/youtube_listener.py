...
# Import app configuration if available
try:
    from .utils.youtube_config import get_youtube_config, YouTubeRetryConfig
...
class EnhancedYouTubeListener:
    ...
    def __init__(
        ...
        Args:
            ...
            use_app_config: Whether to use app configuration system
        """
        # Load app configuration if available and requested
        app_config = None
        if use_app_config and CONFIG_AVAILABLE:
            try:
                app_config = get_youtube_config()
            except Exception as e:
                print(f"Warning: Could not load app config: {e}")

        # Use app config values as defaults if available
        if app_config:
            ...
            self.retry_config = retry_config or RetryConfig(
                ...
                initial_delay=app_config.retry.initial_delay,
                max_delay=app_config.retry.max_delay,
                backoff_multiplier=app_config.retry.backoff_multiplier,
                jitter=app_config.retry.jitter
            )
            log_level = log_level if log_level != "INFO" else app_config.log_level
        else:
...
def create_listener_from_config(youtube_url: str, **kwargs) -> EnhancedYouTubeListener:
    """
    Create an EnhancedYouTubeListener using app configuration.
    ...
    Returns:
        Configured EnhancedYouTubeListener instance
    """
    return EnhancedYouTubeListener(
...
Examples:
  ...
  %(prog)s --url "https://youtu.be/jGpWRfxHFcs" --max-retries 10 --log-level DEBUG
    ...
    listener = EnhancedYouTubeListener(
        ...
        use_app_config=True,  # Use app config when available
    )
